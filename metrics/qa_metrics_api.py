...
def language_score_cul(predict, i):
    predict_answer = get_between_multi(predict['predict'], '<answer>', '</answer>')
    bench_answer = get_between_strings_regex(predict['label'], '<answer>', '</answer>')
    atype = str(predict['atype'])
    language = predict['language']
    ttype = predict['sheet_name']
    score = 0
    # print(atype)
    ...
    for pre_answer in predict_answer:
        if atype == "1":
            ...
            try:
                ...
                # print(e)
                pass
                # print(i,pre_answer,'——————', bench_answer)
...
def get_language_scores(predict_data):
    i = 0
    total_digit_scores = []
    total_locate_scores = []
    ...
    for predict in predict_data:
        i += 1
        # if i == 2094:
        score, atype, ttype = language_score_cul(predict, i)
        if atype == '1':
...
def get_table_type_scores(predict_data):
    i = 0
    total_1_scores = []
    total_2_scores = []
    total_3_scores = []
...
def get_api_content(text):
    sheet_name = get_between_strings_regex(text, "'sheet_name': ", ", 'language'")
    language = get_between_strings_regex(text, "'language': ", ", 'atype'")
    atype = get_between_strings_regex(text, "'atype': ", ", 'label'")
    label = get_between_strings_regex(text, "'label': ", ", 'predict'")
    # print(text)
    try:
        predict = text.split("'predict': ")[1][:-2]
    except:
        predict = ""
...
def main():
    ...
    predict_artificial_path = "/volume/pt-train/users/wzhang/sdx-workspace/output/api/gpt-4o/artificial_data/generated_predictions.jsonl"
    predict_llm_path = "/volume/pt-train/users/wzhang/sdx-workspace/output/api/gpt-4o/llm_data/generated_predictions.jsonl"
    ...
    with open(predict_artificial_path, 'r') as file:
        for line in file:
            pre_data = get_api_content(line)
            predict_artificial_data.append(pre_data)

    predict_llm_data = []
    with open(predict_llm_path, 'r') as file:
        for line in file:
            pre_data = get_api_content(line)
            predict_llm_data.append(pre_data)
    ...
    print("人工构造数据得分：")
    language_scores_artificial = get_language_scores(predict_artificial_data)
    print("LLM生成数据得分：")
    language_scores_llm = get_language_scores(predict_llm_data)

    total_predict = predict_llm_data
    total_predict.extend(predict_artificial_data)
    print(len(total_predict))
    print("总得分：")
    total_scores = get_language_scores(total_predict)

    get_table_type_scores(total_predict)

    language_scores = language_scores_artificial
    for lang in language_scores:
        lang_score = language_scores[lang]
        lang_score_llm = language_scores_llm[lang]
        lang_score['digit_scores'].extend(lang_score_llm['digit_scores'])
        lang_score['locate_scores'].extend(lang_score_llm['locate_scores'])
        lang_score['true_false_scores'].extend(lang_score_llm['true_false_scores'])
        lang_score['nl_scores'].extend(lang_score_llm['nl_scores'])
